...
def load_lora_backbone(lora_ckpt: str, lora_scale: float = 1.0):
    """Load BLIP vision model with LoRA merged (unchanged from v60)."""
    ...
    if lora_scale != 1.0:
        ...
        print(f"  -> LoRA backbone scaled by alpha={lora_scale}")
...
BACKBONE_SPECS = {
    ...
    "dinov2": {
        ...
        "hidden_size": 768,
        # DINOv2 uses standard ImageNet stats
...
class BackboneWrapper(nn.Module):
    """Normalizes the forward interface across BLIP / CLIP / DINOv2.

    Downstream code calls ``self.base_model(pixel_values=x).last_hidden_state[:, 0, :]``,
    which already works for all three HF models. This wrapper is a thin pass-through
    so that the frozen backbone module looks identical to the BLIP case.
    """

    def __init__(self, inner: nn.Module, backbone_name: str):
        super().__init__()
        self.inner = inner
...
def load_backbone(backbone_name: str, lora_ckpt: str = None, lora_scale: float = 1.0):
    """Load one of the supported frozen vision backbones.

    Returns a module whose ``forward(pixel_values=x).last_hidden_state[:, 0, :]``
    yields a (B, 768) CLS embedding.
    """
    name = backbone_name.lower()
    if name == "blip":
        if lora_ckpt:
            vision = load_lora_backbone(lora_ckpt, lora_scale)
        else:
            full = BlipModel.from_pretrained(BACKBONE_SPECS["blip"]["hf_id"])
            vision = full.vision_model
        return BackboneWrapper(vision, "blip")
    if name == "clip":
        vision = CLIPVisionModel.from_pretrained(
            BACKBONE_SPECS["clip"]["hf_id"], use_safetensors=True,
        )
        print(f"  -> loaded CLIP vision backbone: {BACKBONE_SPECS['clip']['hf_id']}")
        return BackboneWrapper(vision, "clip")
    if name == "dinov2":
        vision = AutoModel.from_pretrained(
            BACKBONE_SPECS["dinov2"]["hf_id"], use_safetensors=True,
        )
        print(f"  -> loaded DINOv2 backbone: {BACKBONE_SPECS['dinov2']['hf_id']}")
        return BackboneWrapper(vision, "dinov2")
...
def get_transforms(backbone_name: str = "blip"):
    import torchvision.transforms as T
    spec = BACKBONE_SPECS[backbone_name.lower()]
    return T.Compose([
...
def main():
    ...
    parser.add_argument("--lora_ckpt", type=str, default=None,
                        help="Path to LoRA adapter dir for backbone (only used when --backbone=blip)")
    parser.add_argument("--lora_scale", type=float, default=1.0,
                        help="WiSE-FT alpha for backbone LoRA (0=pretrained, 1=fully finetuned)")
    parser.add_argument("--backbone", type=str, default="blip",
                        choices=["blip", "clip", "dinov2"],
                        help="Frozen vision backbone to build DART on top of")
    ...
    # --- Models ---
    print(f"[Init] Loading backbone: {args.backbone}")
    if args.backbone == "blip" and args.lora_ckpt:
        print(f"[xback] BLIP LoRA backbone: {args.lora_ckpt} (scale={args.lora_scale})")
